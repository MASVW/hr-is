        ...
        <form wire:submit.prevent="save" class="space-y-4">
            <label class="block">
                ...
                <select
                    ...
                    @disabled(!is_null($pic_id)) {{-- kunci jika sudah ada PIC; hapus ini kalau mau bisa ganti --}}
                >
                    <option value="" disabled @selected(empty($pic_id))>— Pilih PIC —</option>
                    @foreach($hrStaff as $staff)
                        <option value="{{ $staff['id'] }}">
                            {{ $staff['name'] }} @if(!empty($staff['email']))
                                — {{ $staff['email'] }}
                            @endif
                        </option>
                    @endforeach
            ...
            <div class="pt-2">
                <button
                    type="submit"
                    @disabled(!is_null($pic_id)) {{-- kunci tombol jika sudah ada PIC --}}
                   ...
                   {{ empty($pic_id) ? 'text-white bg-gradient-to-r from-primary-950 via-primary to-primary hover:bg-gradient-to-l hover:scale-105 transition' : 'text-white bg-zinc-400' }}"
                >
                    Tetapkan PIC
                </button>
            </div>
